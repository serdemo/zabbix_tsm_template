...
function get_failed_baksched {
	...
	[[ "$num_sched" -ne 0 ]] && tsm_output2="$(tsm_cmd -tab "$sql2" | xargs | tr ' ' ';')" && msg="$num_sched on node(s): $tsm_output2" || msg=""
...
function tsm_uptime {
	sql="((days(timestamp(RESTART_DATE)) - days(timestamp('1970-01-01'))) * 86400  + midnight_seconds(timestamp(RESTART_DATE))) as epoch_time"
	tsm_cmd "SELECT $sql FROM status" 2>/dev/null | xargs
}
